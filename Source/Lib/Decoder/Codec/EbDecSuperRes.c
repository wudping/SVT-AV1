...
void av1_upscale_normative_rows(const Av1Common *cm, const uint8_t *src,
                                ...
                                int rows, int sub_x, int bd);

void av1_upscale_normative_and_extend_frame(struct Av1Common *cm,
                                            FrameHeader *frm_hdr,
                                            SeqHeader *seq_hdr,
    ...
    for (int plane = 0; plane < num_planes; ++plane) {
        ...
        derive_blk_pointers(
            src, plane, 0, 0, (void *)&src_buf, &src_stride, sub_x, sub_y);
        derive_blk_pointers(
            dst, plane, 0, 0, (void *)&dst_buf, &dst_stride, sub_x, sub_y);
...
EbErrorType copy_recon(SeqHeader *seq_hdr,
    ...
    if (recon_picture_dst->buffer_enable_mask & PICTURE_BUFFER_DESC_Y_FLAG) {
        EB_ALLIGN_MALLOC_DEC(EbByte,
                             ...
                             EB_A_PTR);
        memset(recon_picture_dst->buffer_y,
               0,
               recon_picture_dst->luma_size * bytesPerPixel);
    } else
    ...
    if (recon_picture_dst->buffer_enable_mask & PICTURE_BUFFER_DESC_Cb_FLAG) {
        EB_ALLIGN_MALLOC_DEC(EbByte,
                             ...
                             EB_A_PTR);
        memset(recon_picture_dst->buffer_cb,
               0,
               recon_picture_dst->chroma_size * bytesPerPixel);
    } else
    ...
    if (recon_picture_dst->buffer_enable_mask & PICTURE_BUFFER_DESC_Cr_FLAG) {
        EB_ALLIGN_MALLOC_DEC(EbByte,
                             ...
                             recon_picture_dst->chroma_size * bytesPerPixel,
                             EB_A_PTR);
        memset(recon_picture_dst->buffer_cr,
               0,
               recon_picture_dst->chroma_size * bytesPerPixel);
    } else
    ...
    for (int plane = 0; plane < num_planes; ++plane) {
        ...
        derive_blk_pointers(recon_picture_src,
                            plane,
                            0,
                            0,
                            (void *)&src_buf,
                            ...
                            sub_x,
                            sub_y);
        derive_blk_pointers(recon_picture_dst,
                            plane,
                            0,
                            0,
                            (void *)&dst_buf,
        ...
        for (int row = 0; row < height; ++row) {
            memcpy(dst_buf,
                   src_buf,
                   (recon_picture_src->width >> sub_x) * sizeof(*src_buf)
                       << use_highbd);
...
void av1_superres_upscale(Av1Common *cm, FrameHeader *frm_hdr,
                          SeqHeader *seq_hdr,
                          EbPictureBufferDesc *recon_picture_src,
                          int enable_flag) {
    if (!enable_flag)
    ...
    memset(recon_picture_src->buffer_y,
           0,
           recon_picture_src->luma_size * bytesPerPixel);
    memset(recon_picture_src->buffer_cb,
           0,
           recon_picture_src->chroma_size * bytesPerPixel);
    memset(recon_picture_src->buffer_cr,
           0,
           recon_picture_src->chroma_size * bytesPerPixel);
