...
/* decode partition */
static void decode_partition(DecModCtxt *dec_mod_ctxt, uint32_t mi_row,
                             uint32_t mi_col, BlockSize bsize,
                             SBInfo *sb_info) {
    BlockSize subsize;
    ...
    uint32_t has_rows =
        (mi_row + half_block_4x4) < dec_mod_ctxt->frame_header->mi_rows;
    uint32_t has_cols =
        (mi_col + half_block_4x4) < dec_mod_ctxt->frame_header->mi_cols;

    if (mi_row >= dec_mod_ctxt->frame_header->mi_rows ||
        mi_col >= dec_mod_ctxt->frame_header->mi_cols)
        return;

    partition = get_partition(dec_mod_ctxt,
...
#define DECODE_PARTITION(db_r, db_c, db_subsize) \
    ...
    case PARTITION_SPLIT:
        ...
        DECODE_PARTITION(
            mi_row + half_block_4x4, mi_col + half_block_4x4, subsize);
        break;
    ...
    case PARTITION_HORZ_B:
        ...
        DECODE_BLOCK(
            mi_row + half_block_4x4, mi_col + half_block_4x4, splitSize);
        break;
    ...
    case PARTITION_HORZ_4:
        for (int i = 0; i < 4; ++i) {
            ...
            if (i > 0 && this_mi_row >= dec_mod_ctxt->frame_header->mi_rows)
                break;
    ...
    case PARTITION_VERT_4:
        for (int i = 0; i < 4; ++i) {
            ...
            if (i > 0 && this_mi_col >= dec_mod_ctxt->frame_header->mi_cols)
                break;
...
void decode_super_block(DecModCtxt *dec_mod_ctxt, uint32_t mi_row,
                        uint32_t mi_col, SBInfo *sb_info) {
    ...
    /* Decode partition */
    decode_partition(dec_mod_ctxt,
                     mi_row,
                     mi_col,
                     dec_mod_ctxt->seq_header->sb_size,
                     sb_info);
}

EbErrorType decode_tile_row(
    ...
    int32_t sb_mi_size_log2 =
        dec_mod_ctxt->seq_header->sb_size_log2 - MI_SIZE_LOG2;
    ...
    if (0 != sb_row_in_tile) {
        sb_completed_in_prev_row =
            (volatile int32_t *)&parse_recon_tile_info_array
                ->sb_recon_completed_in_row[sb_row_in_tile - 1];
    }

    sb_completed_in_row =
        &parse_recon_tile_info_array->sb_recon_completed_in_row[sb_row_in_tile];
    ...
    // tile_wd_in_sb = ( (tile_info->tile_col_start_mi[tile_col + 1] <<
    // MI_SIZE_LOG2) ) >>
    //   dec_mod_ctxt->seq_header->sb_size_log2;
#endif
    ...
    {
        int32_t sb_col =
            (mi_col << MI_SIZE_LOG2) >> dec_mod_ctxt->seq_header->sb_size_log2;

        SBInfo *sb_info =
            frame_buf->sb_info + (sb_row * master_frame_buf->sb_cols) + sb_col;
    ...
    mt_frame_data->sb_recon_row_map[(index * tile_info->tile_cols) + tile_col] =
        1;
    return status;
}
...
EbErrorType decode_tile(DecModCtxt *dec_mod_ctxt, TilesInfo *tile_info,
    ...
    for (uint32_t mi_row = tile_info->tile_row_start_mi[tile_row];
        ...
        int32_t sb_row =
            (mi_row << MI_SIZE_LOG2) >> dec_mod_ctxt->seq_header->sb_size_log2;
        status = decode_tile_row(dec_mod_ctxt,
                                 tile_info,
                                 parse_recon_tile_info_array,
                                 tile_col,
                                 mi_row,
                                 sb_row);
    }
...
EbErrorType decode_tile(DecModCtxt *dec_mod_ctxt, TilesInfo *tile_info,
    ...
    while (1) {
        ...
        int32_t sb_row_tile_start =
            (parse_recon_tile_info_array->tile_info.mi_row_start
             << MI_SIZE_LOG2) >>
            dec_mod_ctxt->seq_header->sb_size_log2;
        ...
        // wait for parse
        if (-1 != sb_row_in_tile) {
            volatile int32_t *sb_row_parsed =
                (volatile int32_t *)&parse_recon_tile_info_array
            ...
            mi_row = (sb_row << dec_mod_ctxt->seq_header->sb_size_log2) >>
                     MI_SIZE_LOG2;
            ...
            parse_recon_tile_info_array->sb_recon_row_started[sb_row_in_tile] =
                1;
        ...
        /*if all sb rows have been picked up for processing then break the while
         * loop */
        if (parse_recon_tile_info_array->sb_row_to_process ==
            parse_recon_tile_info_array->tile_num_sb_rows) {
            break;
...
EbErrorType start_decode_tile(EbDecHandle *dec_handle_ptr,
                              DecModCtxt *dec_mod_ctxt, TilesInfo *tiles_info,
                              int32_t tile_num) {
    EbErrorType status = EB_ErrorNone;
    ...
    svt_tile_init(
        &dec_mod_ctxt->cur_tile_info, frame_header, tile_row, tile_col);

    parse_recon_tile_info_array =
        &dec_mt_frame_data->parse_recon_tile_info_array[tile_num];

#if !ENABLE_ROW_MT_DECODE
    status = decode_tile(dec_mod_ctxt,
                         tiles_info,
                         parse_recon_tile_info_array,
                         tile_row,
                         tile_col);
#else
    status = decode_tile(
        dec_mod_ctxt, tiles_info, parse_recon_tile_info_array, tile_col);
#endif
