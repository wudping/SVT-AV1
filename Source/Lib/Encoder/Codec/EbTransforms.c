...
void eb_av1_fidentity4_c(const int32_t *input, int32_t *output, int8_t cos_bit,
    ...
    for (int32_t i = 0; i < 4; ++i)
        output[i] = round_shift((int64_t)input[i] * new_sqrt2, NewSqrt2Bits);
...
void eb_av1_fidentity16_c(const int32_t *input, int32_t *output, int8_t cos_bit,
    ...
    for (int32_t i = 0; i < 16; ++i)
        output[i] = round_shift((int64_t)input[i] * 2 * new_sqrt2, NewSqrt2Bits);
    assert(stage_range[0] + NewSqrt2Bits <= 32);
...
void av1_fidentity64_c(const int32_t *input, int32_t *output, int8_t cos_bit,
    ...
    for (int32_t i = 0; i < 64; ++i)
        output[i] = round_shift((int64_t)input[i] * 4 * new_sqrt2, NewSqrt2Bits);
    assert(stage_range[0] + NewSqrt2Bits <= 32);
        ...
        if (abs(rect_type) == 1) {
            ...
            for (c = 0; c < txfm_size_col; ++c) {
                output[r * txfm_size_col + c] = round_shift(
                    (int64_t)output[r * txfm_size_col + c] * new_sqrt2, NewSqrt2Bits);
            }
        }
    }
}
...
void eb_av1_iidentity4_c(const int32_t *input, int32_t *output, int8_t cos_bit,
    ...
    for (int32_t i = 0; i < 4; ++i) {
        ...
        output[i] = round_shift((int64_t)new_sqrt2 * input[i], NewSqrt2Bits);
    }
    assert(stage_range[0] + NewSqrt2Bits <= 32);
}
...
void eb_av1_iidentity16_c(const int32_t *input, int32_t *output, int8_t cos_bit,
    ...
    (void)stage_range;
    for (int32_t i = 0; i < 16; ++i)
        output[i] = round_shift((int64_t)new_sqrt2 * 2 * input[i], NewSqrt2Bits);
    assert(stage_range[0] + NewSqrt2Bits <= 32);
}
...
void av1_iidentity64_c(const int32_t *input, int32_t *output, int8_t cos_bit,
    ...
    (void)stage_range;
    for (int32_t i = 0; i < 64; ++i)
        output[i] = round_shift((int64_t)new_sqrt2 * 4 * input[i], NewSqrt2Bits);
    assert(stage_range[0] + NewSqrt2Bits <= 32);
}
    ...
    int32_t bd)
{
    ...
    for (r = 0; r < txfm_size_row; ++r) {
        if (abs(rect_type) == 1) {
            for (c = 0; c < txfm_size_col; ++c)
                temp_in[c] = round_shift((int64_t)input[c] * new_inv_sqrt2, NewSqrt2Bits);
