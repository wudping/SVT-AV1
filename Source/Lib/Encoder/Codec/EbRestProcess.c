    ...
    int                 demux_index)
{
...

    {
        ...
        initData.max_width = (uint16_t)scs_ptr->max_input_luma_width;
        initData.max_height = (uint16_t)scs_ptr->max_input_luma_height;
    ...
    tempLfReconDescInitData.max_width = (uint16_t)scs_ptr->max_input_luma_width;
    tempLfReconDescInitData.max_height = (uint16_t)scs_ptr->max_input_luma_height;
    ...
    EbBool  is16bit)
{
    ...
    if (is16bit) {
        if (pcs_ptr->parent_pcs_ptr->is_used_as_reference_flag == EB_TRUE)
            recon_picture_ptr = ((EbReferenceObject*)pcs_ptr->parent_pcs_ptr->reference_picture_wrapper_ptr->object_ptr)->reference_picture16bit;
        else
        ...
        for (int r = 0; r < scs_ptr->seq_header.max_frame_height; ++r)
            memcpy(org_ptr + r * org_rec->stride_y, rec_ptr + r * recon_picture_ptr->stride_y, scs_ptr->seq_header.max_frame_width << 1);

        for (int r = 0; r < scs_ptr->seq_header.max_frame_height / 2; ++r) {
            memcpy(org_ptr_cb + r * org_rec->stride_cb, rec_ptr_cb + r * recon_picture_ptr->stride_cb, (scs_ptr->seq_header.max_frame_width / 2) << 1);
            memcpy(org_ptr_cr + r * org_rec->stride_cr, rec_ptr_cr + r * recon_picture_ptr->stride_cr, (scs_ptr->seq_header.max_frame_width / 2) << 1);
        }
    }
    else {
        if (pcs_ptr->parent_pcs_ptr->is_used_as_reference_flag == EB_TRUE)
            recon_picture_ptr = ((EbReferenceObject*)pcs_ptr->parent_pcs_ptr->reference_picture_wrapper_ptr->object_ptr)->reference_picture;
        else
        ...
        for (int r = 0; r < scs_ptr->seq_header.max_frame_height; ++r)
            memcpy(org_ptr + r * org_rec->stride_y, rec_ptr + r * recon_picture_ptr->stride_y, scs_ptr->seq_header.max_frame_width);

        for (int r = 0; r < scs_ptr->seq_header.max_frame_height / 2; ++r) {
            memcpy(org_ptr_cb + r * org_rec->stride_cb, rec_ptr_cb + r * recon_picture_ptr->stride_cb, (scs_ptr->seq_header.max_frame_width / 2));
            memcpy(org_ptr_cr + r * org_rec->stride_cr, rec_ptr_cr + r * recon_picture_ptr->stride_cr, (scs_ptr->seq_header.max_frame_width / 2));
        }
    }
}
...
void* rest_kernel(void *input_ptr)
{
    ...
    for (;;) {
        ...
        pcs_ptr = (PictureControlSet*)cdef_results_ptr->picture_control_set_wrapper_ptr->object_ptr;
        scs_ptr = (SequenceControlSet*)pcs_ptr->sequence_control_set_wrapper_ptr->object_ptr;
        frm_hdr = &pcs_ptr->parent_pcs_ptr->frm_hdr;
        uint8_t lcuSizeLog2 = (uint8_t)Log2f(scs_ptr->sb_size_pix);
        EbBool  is16bit = (EbBool)(scs_ptr->static_config.encoder_bit_depth > EB_8BIT);
        Av1Common* cm = pcs_ptr->parent_pcs_ptr->av1_cm;

        if (scs_ptr->seq_header.enable_restoration && frm_hdr->allow_intrabc == 0)
        {
            get_own_recon(scs_ptr, pcs_ptr, context_ptr, is16bit);

            Yv12BufferConfig cpi_source;
            link_eb_to_aom_buffer_desc(
                is16bit ? pcs_ptr->input_frame16bit : pcs_ptr->parent_pcs_ptr->enhanced_picture_ptr,
                &cpi_source);
        ...
        if (pcs_ptr->tot_seg_searched_rest == pcs_ptr->rest_segments_total_count)
        {
            ...
            if (pcs_ptr->parent_pcs_ptr->is_used_as_reference_flag)
            {
                ...
                picture_demux_results_rtr->reference_picture_wrapper_ptr = pcs_ptr->parent_pcs_ptr->reference_picture_wrapper_ptr;
                picture_demux_results_rtr->sequence_control_set_wrapper_ptr = pcs_ptr->sequence_control_set_wrapper_ptr;
            ...
            rest_results_ptr->completed_lcu_row_count = ((scs_ptr->seq_header.max_frame_height + scs_ptr->sb_size_pix - 1) >> lcuSizeLog2);
            // Post Rest Results
