    ...
    EbPtr object_init_data_ptr)
{
    ...
    if (scsInitData != EB_NULL)
        scs_ptr->encode_context_ptr = scsInitData->encode_context_ptr;
    ...
    // CU
    scs_ptr->max_cu_size = 64;
    scs_ptr->min_cu_size = 8;
    ...
    scs_ptr->seq_header.order_hint_info.order_hint_bits = scs_ptr->seq_header.order_hint_info.enable_order_hint ? (6+1) : (-1+1);
    ...
    EbSequenceControlSetInstance *object_dbl_ptr)
{
    ...
    EB_NEW(object_dbl_ptr->scs_ptr, eb_sequence_control_set_ctor, (void *)&scsInitData);
...
extern EbErrorType sb_params_init(
    ...
    uint8_t   pictureLcuWidth = (uint8_t)((scs_ptr->seq_header.max_frame_width + scs_ptr->sb_sz - 1) / scs_ptr->sb_sz);
    uint8_t    pictureLcuHeight = (uint8_t)((scs_ptr->seq_header.max_frame_height + scs_ptr->sb_sz - 1) / scs_ptr->sb_sz);
    //free old one;
    EB_FREE_ARRAY(scs_ptr->sb_params_array);

    EB_MALLOC_ARRAY(scs_ptr->sb_params_array, pictureLcuWidth * pictureLcuHeight);

    for (sb_index = 0; sb_index < pictureLcuWidth * pictureLcuHeight; ++sb_index) {
        scs_ptr->sb_params_array[sb_index].horizontal_index = (uint8_t)(sb_index % pictureLcuWidth);
        scs_ptr->sb_params_array[sb_index].vertical_index = (uint8_t)(sb_index / pictureLcuWidth);
        scs_ptr->sb_params_array[sb_index].origin_x = scs_ptr->sb_params_array[sb_index].horizontal_index * scs_ptr->sb_sz;
        scs_ptr->sb_params_array[sb_index].origin_y = scs_ptr->sb_params_array[sb_index].vertical_index * scs_ptr->sb_sz;

        scs_ptr->sb_params_array[sb_index].width = (uint8_t)(((scs_ptr->seq_header.max_frame_width - scs_ptr->sb_params_array[sb_index].origin_x) < scs_ptr->sb_sz) ?
            scs_ptr->seq_header.max_frame_width - scs_ptr->sb_params_array[sb_index].origin_x :
            scs_ptr->sb_sz);

        scs_ptr->sb_params_array[sb_index].height = (uint8_t)(((scs_ptr->seq_header.max_frame_height - scs_ptr->sb_params_array[sb_index].origin_y) < scs_ptr->sb_sz) ?
            scs_ptr->seq_header.max_frame_height - scs_ptr->sb_params_array[sb_index].origin_y :
            scs_ptr->sb_sz);

        scs_ptr->sb_params_array[sb_index].is_complete_sb = (uint8_t)(((scs_ptr->sb_params_array[sb_index].width == scs_ptr->sb_sz) && (scs_ptr->sb_params_array[sb_index].height == scs_ptr->sb_sz)) ?
            1 :
            0);

        scs_ptr->sb_params_array[sb_index].is_edge_sb = (scs_ptr->sb_params_array[sb_index].origin_x < scs_ptr->sb_sz) ||
            (scs_ptr->sb_params_array[sb_index].origin_y < scs_ptr->sb_sz) ||
            (scs_ptr->sb_params_array[sb_index].origin_x > scs_ptr->seq_header.max_frame_width - scs_ptr->sb_sz) ||
            (scs_ptr->sb_params_array[sb_index].origin_y > scs_ptr->seq_header.max_frame_height - scs_ptr->sb_sz) ? 1 : 0;
        ...
        if (scs_ptr->input_resolution <= INPUT_SIZE_576p_RANGE_OR_LOWER) {
            potential_logo_sb = (scs_ptr->sb_params_array[sb_index].origin_x >= (scs_ptr->seq_header.max_frame_width - (3 * scs_ptr->sb_sz))) && (scs_ptr->sb_params_array[sb_index].origin_y < 2 * scs_ptr->sb_sz) ? 1 : potential_logo_sb;
            potential_logo_sb = (scs_ptr->sb_params_array[sb_index].origin_x < ((3 * scs_ptr->sb_sz))) && (scs_ptr->sb_params_array[sb_index].origin_y < 2 * scs_ptr->sb_sz) ? 1 : potential_logo_sb;
            potential_logo_sb = (scs_ptr->sb_params_array[sb_index].origin_y >= (scs_ptr->seq_header.max_frame_height - (2 * scs_ptr->sb_sz))) ? 1 : potential_logo_sb;
        }
        else
            if (scs_ptr->input_resolution < INPUT_SIZE_4K_RANGE) {
                potential_logo_sb = (scs_ptr->sb_params_array[sb_index].origin_x >= (scs_ptr->seq_header.max_frame_width - (7 * scs_ptr->sb_sz))) && (scs_ptr->sb_params_array[sb_index].origin_y < 4 * scs_ptr->sb_sz) ? 1 : potential_logo_sb;
                potential_logo_sb = (scs_ptr->sb_params_array[sb_index].origin_x < ((7 * scs_ptr->sb_sz))) && (scs_ptr->sb_params_array[sb_index].origin_y < 4 * scs_ptr->sb_sz) ? 1 : potential_logo_sb;
                potential_logo_sb = (scs_ptr->sb_params_array[sb_index].origin_y >= (scs_ptr->seq_header.max_frame_height - (4 * scs_ptr->sb_sz))) ? 1 : potential_logo_sb;
            }
            else {
                potential_logo_sb = (scs_ptr->sb_params_array[sb_index].origin_x >= (scs_ptr->seq_header.max_frame_width - (14 * scs_ptr->sb_sz))) && (scs_ptr->sb_params_array[sb_index].origin_y < 8 * scs_ptr->sb_sz) ? 1 : potential_logo_sb;
                potential_logo_sb = (scs_ptr->sb_params_array[sb_index].origin_x < ((14 * scs_ptr->sb_sz))) && (scs_ptr->sb_params_array[sb_index].origin_y < 8 * scs_ptr->sb_sz) ? 1 : potential_logo_sb;
                potential_logo_sb = (scs_ptr->sb_params_array[sb_index].origin_y >= (scs_ptr->seq_header.max_frame_height - (8 * scs_ptr->sb_sz))) ? 1 : potential_logo_sb;
            }
        ...
        for (rasterScanCuIndex = RASTER_SCAN_CU_INDEX_64x64; rasterScanCuIndex <= RASTER_SCAN_CU_INDEX_8x8_63; rasterScanCuIndex++) {
            scs_ptr->sb_params_array[sb_index].raster_scan_cu_validity[rasterScanCuIndex] = ((scs_ptr->sb_params_array[sb_index].origin_x + raster_scan_cu_x[rasterScanCuIndex] + raster_scan_cu_size[rasterScanCuIndex] > scs_ptr->seq_header.max_frame_width) || (scs_ptr->sb_params_array[sb_index].origin_y + raster_scan_cu_y[rasterScanCuIndex] + raster_scan_cu_size[rasterScanCuIndex] > scs_ptr->seq_header.max_frame_height)) ?
                EB_FALSE :
                EB_TRUE;
        }
    }

    scs_ptr->picture_width_in_sb = pictureLcuWidth;
    scs_ptr->picture_height_in_sb = pictureLcuHeight;
    scs_ptr->sb_total_count = pictureLcuWidth * pictureLcuHeight;
...
EbErrorType sb_geom_init(SequenceControlSet * scs_ptr)
{
    ...
    uint16_t   pictureLcuWidth = (scs_ptr->seq_header.max_frame_width + scs_ptr->sb_size_pix - 1) / scs_ptr->sb_size_pix;
    uint16_t    pictureLcuHeight = (scs_ptr->seq_header.max_frame_height + scs_ptr->sb_size_pix - 1) / scs_ptr->sb_size_pix;


    EB_FREE_ARRAY(scs_ptr->sb_geom);
    EB_MALLOC_ARRAY(scs_ptr->sb_geom, pictureLcuWidth * pictureLcuHeight);

    for (sb_index = 0; sb_index < pictureLcuWidth * pictureLcuHeight; ++sb_index) {
        scs_ptr->sb_geom[sb_index].horizontal_index = sb_index % pictureLcuWidth;
        scs_ptr->sb_geom[sb_index].vertical_index = sb_index / pictureLcuWidth;
        scs_ptr->sb_geom[sb_index].origin_x = scs_ptr->sb_geom[sb_index].horizontal_index * scs_ptr->sb_size_pix;
        scs_ptr->sb_geom[sb_index].origin_y = scs_ptr->sb_geom[sb_index].vertical_index * scs_ptr->sb_size_pix;

        scs_ptr->sb_geom[sb_index].width = (uint8_t)(((scs_ptr->seq_header.max_frame_width - scs_ptr->sb_geom[sb_index].origin_x) < scs_ptr->sb_size_pix) ?
            scs_ptr->seq_header.max_frame_width - scs_ptr->sb_geom[sb_index].origin_x :
            scs_ptr->sb_size_pix);

        scs_ptr->sb_geom[sb_index].height = (uint8_t)(((scs_ptr->seq_header.max_frame_height - scs_ptr->sb_geom[sb_index].origin_y) < scs_ptr->sb_size_pix) ?
            scs_ptr->seq_header.max_frame_height - scs_ptr->sb_geom[sb_index].origin_y :
            scs_ptr->sb_size_pix);

        scs_ptr->sb_geom[sb_index].is_complete_sb =
            (uint8_t)(((scs_ptr->sb_geom[sb_index].width == scs_ptr->sb_size_pix) && (scs_ptr->sb_geom[sb_index].height == scs_ptr->sb_size_pix)) ?
                1 :
                0);
        ...
        for (md_scan_block_index = 0; md_scan_block_index < max_block_count ; md_scan_block_index++) {
            ...
            if (scs_ptr->over_boundary_block_mode == 1) {
                scs_ptr->sb_geom[sb_index].block_is_allowed[md_scan_block_index] =
                    ((scs_ptr->sb_geom[sb_index].origin_x + blk_geom->origin_x + blk_geom->bwidth / 2 < scs_ptr->seq_header.max_frame_width) &&
                    (scs_ptr->sb_geom[sb_index].origin_y + blk_geom->origin_y + blk_geom->bheight / 2 < scs_ptr->seq_header.max_frame_height)) ?
                    EB_TRUE :
                    EB_FALSE;
                ...
                int32_t cropped_width = MIN(blk_geom->bwidth, scs_ptr->seq_header.max_frame_width - (scs_ptr->sb_geom[sb_index].origin_x + blk_geom->origin_x));
                ...
                scs_ptr->sb_geom[sb_index].block_is_allowed[md_scan_block_index] =
                    ((scs_ptr->sb_geom[sb_index].origin_x + blk_geom->origin_x + blk_geom->bwidth > scs_ptr->seq_header.max_frame_width) ||
                    (scs_ptr->sb_geom[sb_index].origin_y + blk_geom->origin_y + blk_geom->bheight > scs_ptr->seq_header.max_frame_height)) ?
                    EB_FALSE :
                    EB_TRUE;
    ...
    scs_ptr->sb_tot_cnt = pictureLcuWidth * pictureLcuHeight;
