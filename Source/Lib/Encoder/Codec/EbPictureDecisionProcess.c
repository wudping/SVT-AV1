    ...
    uint32_t                                windowWidthFuture)
{
    ...
    uint32_t  regionCountThreshold = (scs_ptr->scd_mode == SCD_MODE_2) ?
    ...
    regionWidth = ParentPcsWindow[1]->enhanced_picture_ptr->width / scs_ptr->picture_analysis_number_of_regions_per_width;
    regionHeight = ParentPcsWindow[1]->enhanced_picture_ptr->height / scs_ptr->picture_analysis_number_of_regions_per_height;
    ...
    for (regionInPictureWidthIndex = 0; regionInPictureWidthIndex < scs_ptr->picture_analysis_number_of_regions_per_width; regionInPictureWidthIndex++) {  // loop over horizontal regions
        for (regionInPictureHeightIndex = 0; regionInPictureHeightIndex < scs_ptr->picture_analysis_number_of_regions_per_height; regionInPictureHeightIndex++) { // loop over vertical regions
            ...
            regionWidthOffset = (regionInPictureWidthIndex == scs_ptr->picture_analysis_number_of_regions_per_width - 1) ?
                ParentPcsWindow[1]->enhanced_picture_ptr->width - (scs_ptr->picture_analysis_number_of_regions_per_width * regionWidth) :
                0;

            regionHeightOffset = (regionInPictureHeightIndex == scs_ptr->picture_analysis_number_of_regions_per_height - 1) ?
                ParentPcsWindow[1]->enhanced_picture_ptr->height - (scs_ptr->picture_analysis_number_of_regions_per_height * regionHeight) :
                0;
...
EbErrorType GenerateMiniGopRps(
    ...
    for (mini_gop_index = 0; mini_gop_index < context_ptr->total_number_of_mini_gops; ++mini_gop_index) {
        ...
        for (pictureIndex = context_ptr->mini_gop_start_index[mini_gop_index]; pictureIndex <= context_ptr->mini_gop_end_index[mini_gop_index]; pictureIndex++) {
            pcs_ptr = (PictureParentControlSet*)encode_context_ptr->pre_assignment_buffer[pictureIndex]->object_ptr;
        ...
        if (sc_content_detected)
            ...
            if (pcs_ptr->enc_mode <= ENC_M1)
                ...
                if (pcs_ptr->temporal_layer_index == 0)
                    pcs_ptr->pic_depth_mode = PIC_ALL_DEPTH_MODE;
                else if (pcs_ptr->is_used_as_reference_flag)
...
// These are the default settings to use
        ...
        if (scs_ptr->static_config.olpd_refinement == -1) { //auto mode; if not set by cfg
            if (pcs_ptr->pic_depth_mode <= PIC_ALL_C_DEPTH_MODE) {
                if (MR_MODE || sc_content_detected)
                    pcs_ptr->enable_adaptive_ol_partitioning = 0;
                else if (pcs_ptr->enc_mode <= ENC_M0)
                    pcs_ptr->enable_adaptive_ol_partitioning = 1;
                else
                    pcs_ptr->enable_adaptive_ol_partitioning = 0;
            }
            else
            ...
            pcs_ptr->enable_adaptive_ol_partitioning = scs_ptr->static_config.olpd_refinement;
        }
#else
    ...
    // 7                                     All
    if (MR_MODE || sc_content_detected)
        pcs_ptr->mdc_depth_level = MAX_MDC_LEVEL;
    else if (pcs_ptr->enc_mode == ENC_M0)
        pcs_ptr->mdc_depth_level = (scs_ptr->input_resolution == INPUT_SIZE_576p_RANGE_OR_LOWER) ? MAX_MDC_LEVEL : 6;
    else
        pcs_ptr->mdc_depth_level = MAX_MDC_LEVEL; // Not tuned yet.
    ...
    // NSQ_SEARCH_FULL                                Allow NSQ Intra-FULL and Inter-FULL
        ...
        if (MR_MODE)
            ...
            if (pcs_ptr->enc_mode <= ENC_M1)
                ...
                if (pcs_ptr->temporal_layer_index == 0)
                    ...
                    pcs_ptr->nsq_search_level = NSQ_SEARCH_LEVEL4;
                else
                    pcs_ptr->nsq_search_level = NSQ_SEARCH_OFF;
            else if (pcs_ptr->enc_mode <= ENC_M3)
                if (pcs_ptr->temporal_layer_index == 0)
        ...
        else if (pcs_ptr->mdc_depth_level == (MAX_MDC_LEVEL - 1))
            pcs_ptr->nsq_search_level = NSQ_SEARCH_LEVEL7;
#endif
#if PRESETS_TUNE
        else if (pcs_ptr->enc_mode <= ENC_M0)
            pcs_ptr->nsq_search_level = NSQ_SEARCH_LEVEL6;
    ...
    // 4                                              Interpolation search at fast loop
        ...
        if (MR_MODE)
            pcs_ptr->interpolation_search_level = IT_SEARCH_FAST_LOOP;
        else if (sc_content_detected)
            pcs_ptr->interpolation_search_level = IT_SEARCH_OFF;
        else if (pcs_ptr->enc_mode <= ENC_M1)
            pcs_ptr->interpolation_search_level = IT_SEARCH_FAST_LOOP_UV_BLIND;
        else if (pcs_ptr->enc_mode <= ENC_M3)
            if (pcs_ptr->is_used_as_reference_flag)
                pcs_ptr->interpolation_search_level = IT_SEARCH_FAST_LOOP_UV_BLIND;
            else
    ...
    */
...
#else
            pcs_ptr->cdef_filter_mode = (pcs_ptr->enc_mode <= ENC_M1) ? 5 : 4;
#endif
#else
            pcs_ptr->cdef_filter_mode = (pcs_ptr->enc_mode <= ENC_M0) ? 5 : 4;
#endif
#else
            pcs_ptr->cdef_filter_mode = 4;
#endif
    ...
    if (sc_content_detected)
        if (pcs_ptr->enc_mode <= ENC_M6)
            pcs_ptr->tx_search_level = TX_SEARCH_FULL_LOOP;
        else
            if (pcs_ptr->is_used_as_reference_flag)
                pcs_ptr->tx_search_level = TX_SEARCH_FULL_LOOP;
            else
                pcs_ptr->tx_search_level = TX_SEARCH_ENC_DEC;
    else
    if (pcs_ptr->enc_mode <= ENC_M4)
        pcs_ptr->tx_search_level = TX_SEARCH_FULL_LOOP;
    else if (pcs_ptr->enc_mode <= ENC_M7) {
    ...
    if (pcs_ptr->tx_search_level == TX_SEARCH_ENC_DEC)
        pcs_ptr->tx_weight = MAX_MODE_COST;
    else if (!MR_MODE && pcs_ptr->enc_mode <= ENC_M1)
        pcs_ptr->tx_weight = FC_SKIP_TX_SR_TH025;
    else if (!MR_MODE){
        if (pcs_ptr->is_used_as_reference_flag)
            pcs_ptr->tx_weight = FC_SKIP_TX_SR_TH025;
        else
            pcs_ptr->tx_weight = FC_SKIP_TX_SR_TH010;
    }

    // Set tx search reduced set falg (0: full tx set; 1: reduced tx set; 1: two tx))
    if (sc_content_detected)
        if (pcs_ptr->enc_mode <= ENC_M1)
            pcs_ptr->tx_search_reduced_set = 0;
        else if (pcs_ptr->enc_mode <= ENC_M6)
            if (pcs_ptr->tx_search_level == TX_SEARCH_ENC_DEC)
                pcs_ptr->tx_search_reduced_set = 0;
            else
                pcs_ptr->tx_search_reduced_set = 1;
        else if (pcs_ptr->enc_mode <= ENC_M7)
            pcs_ptr->tx_search_reduced_set = 1;
        else
            pcs_ptr->tx_search_reduced_set = 2;
    else
...
#else
                pcs_ptr->compound_mode = pcs_ptr->sc_content_detected ? 0 :
                pcs_ptr->enc_mode <= ENC_M1 ? 2 : 1;
#endif
            else
                pcs_ptr->compound_mode = 0;
#if !MULTI_PASS_PD
            // set compound_types_to_try
            if (pcs_ptr->compound_mode)
                pcs_ptr->compound_types_to_try = pcs_ptr->compound_mode == 1 ? MD_COMP_DIFF0 : MD_COMP_WEDGE;
            else
                pcs_ptr->compound_types_to_try = MD_COMP_AVG;
#endif
        }
        else
...
#if M0_OPT
            if (pcs_ptr->sc_content_detected || pcs_ptr->enc_mode >= ENC_M4)
#else
            if (pcs_ptr->sc_content_detected || pcs_ptr->enc_mode == ENC_M0 || pcs_ptr->enc_mode >= ENC_M4)
#endif
...
static __inline uint32_t compute_luma_sad_between_center_and_target_frame(
    ...
    for (int bin = 0; bin < HISTOGRAM_NUMBER_OF_BINS; ++bin) {
        ...
        for (uint32_t regionInPictureWidthIndex = 0; regionInPictureWidthIndex < scs_ptr->picture_analysis_number_of_regions_per_width; regionInPictureWidthIndex++) {
            for (uint32_t regionInPictureHeightIndex = 0; regionInPictureHeightIndex < scs_ptr->picture_analysis_number_of_regions_per_height; regionInPictureHeightIndex++) {
                center_sum += pcs_ptr->temp_filt_pcs_list[center_index]->picture_histogram[regionInPictureWidthIndex][regionInPictureHeightIndex][0][bin];
                altref_sum += pcs_ptr->temp_filt_pcs_list[target_frame_index]->picture_histogram[regionInPictureWidthIndex][regionInPictureHeightIndex][0][bin];
            }
        }
...
void* picture_decision_kernel(void *input_ptr)
{
    ...
    for (;;) {
        ...
        in_results_ptr = (PictureAnalysisResults*)inputResultsWrapperPtr->object_ptr;
        ...
        if (!pcs_ptr->is_overlay ) {
            queueEntryIndex = (int32_t)(pcs_ptr->picture_number - encode_context_ptr->picture_decision_reorder_queue[encode_context_ptr->picture_decision_reorder_queue_head_index]->picture_number);
            ...
            pcs_ptr->pic_decision_reorder_queue_idx = queueEntryIndex;
        }
        ...
        while (queueEntryPtr->parent_pcs_wrapper_ptr != EB_NULL) {
            ...
            if (queueEntryPtr->picture_number > 0 && encode_context_ptr->picture_decision_reorder_queue[previousEntryIndex]->parent_pcs_wrapper_ptr == NULL)
                ...
                for (windowIndex = 0; windowIndex < scs_ptr->scd_delay; windowIndex++) {
            ...
            if (windowAvail == EB_TRUE && queueEntryPtr->picture_number > 0) {
                if (scs_ptr->static_config.scene_change_detection) {
                    pcs_ptr->scene_change_flag = SceneTransitionDetector(
                        context_ptr,
                        scs_ptr,
                        ParentPcsWindow,
                        FUTURE_WINDOW_WIDTH);
                    ...
                    for (mini_gop_index = 0; mini_gop_index < context_ptr->total_number_of_mini_gops; ++mini_gop_index) {
                        ...
                        for (pictureIndex = context_ptr->mini_gop_start_index[mini_gop_index]; pictureIndex <= context_ptr->mini_gop_end_index[mini_gop_index]; ++pictureIndex) {
                                ...
                                pcs_ptr->cra_flag == EB_TRUE)
                            {
                                ...
                                picture_type =
                                    ...
                                    (pcs_ptr->pre_assignment_buffer_count == pcs_ptr->pred_struct_ptr->pred_struct_period) ? ((pictureIndex == context_ptr->mini_gop_end_index[mini_gop_index] && scs_ptr->static_config.base_layer_switch_mode) ? P_SLICE : B_SLICE) :
                            ...
                            //    }
                            //    else {
                            //        encode_context_ptr->pred_struct_position = pcs_ptr->pred_struct_ptr->init_pic_index + encode_context_ptr->pre_assignment_buffer_count - bufferIndex - 1;
                            //    }
                            //}
                            ...
                            predPositionPtr = pcs_ptr->pred_struct_ptr->pred_struct_entry_ptr_array[encode_context_ptr->pred_struct_position];
                            ...
                            for (uint8_t loop_index = 0; loop_index <= pcs_ptr->is_alt_ref; loop_index++) {
                                ...
                                if (pcs_ptr->is_overlay) {
                                    ...
                                    pcs_ptr->temporal_layer_index = (uint8_t)predPositionPtr->temporal_layer_index;
                                    pcs_ptr->is_used_as_reference_flag = predPositionPtr->is_referenced;
                                }
                            ...
                            frm_hdr->tx_mode = (pcs_ptr->atb_mode) ?
                                ...
                                if (pcs_ptr->slice_type == I_SLICE)
                                {
                                    ...
                                    while (inputQueueIndex != encode_context_ptr->picture_decision_pa_reference_queue_tail_index) {
                                        ...
                                        for (depIdx = 0; depIdx < depListCount; ++depIdx) {
                                            ...
                                            // current_input_poc += (current_input_poc < inputEntryPtr->pocNumber) ? (1 << scs_ptr->bits_for_picture_order_count) : 0;
                                            ...
                                            depPoc = POC_CIRCULAR_ADD(
                                                ...
                                                // If Dependent POC is greater or equal to the IDR POC
                                            if (depPoc >= pcs_ptr->picture_number && inputEntryPtr->list0.list[depIdx]) {
                                        ...
                                        for (depIdx = 0; depIdx < depListCount; ++depIdx) {
                                            ...
                                            depPoc = POC_CIRCULAR_ADD(
                                                ...
                                                // If Dependent POC is greater or equal to the IDR POC
                                            if (((depPoc >= pcs_ptr->picture_number) || (((pcs_ptr->pre_assignment_buffer_count != pcs_ptr->pred_struct_ptr->pred_struct_period) || (pcs_ptr->idr_flag == EB_TRUE)) && (depPoc > (pcs_ptr->picture_number - pcs_ptr->pre_assignment_buffer_count)))) && inputEntryPtr->list1.list[depIdx]) {
                                ...
                                if (!pcs_ptr->is_overlay)
                                {
                                    ...
                                    inputEntryPtr->input_object_ptr = pcs_ptr->pa_reference_picture_wrapper_ptr;
                                    inputEntryPtr->picture_number = pcs_ptr->picture_number;
                                    inputEntryPtr->reference_entry_index = encode_context_ptr->picture_decision_pa_reference_queue_tail_index;
                                    inputEntryPtr->is_alt_ref = pcs_ptr->is_alt_ref;
                                ...
                                pcs_ptr->ref_list0_count = (picture_type == I_SLICE) ? 0 :
                                                                            (pcs_ptr->is_overlay) ? 1 : (uint8_t)predPositionPtr->ref_list0.reference_list_count;
                                pcs_ptr->ref_list1_count = (picture_type == I_SLICE || pcs_ptr->is_overlay) ? 0 : (uint8_t)predPositionPtr->ref_list1.reference_list_count;
                                if (!pcs_ptr->is_overlay) {
                                ...
                                if (!pcs_ptr->is_overlay)
                                {
                                    ...
                                    inputEntryPtr->dep_list0_count = (pcs_ptr->is_alt_ref) ? inputEntryPtr->list0.list_count + 1 : inputEntryPtr->list0.list_count;
                                    ...
                                    ((EbPaReferenceObject*)pcs_ptr->pa_reference_picture_wrapper_ptr->object_ptr)->dependent_pictures_count = inputEntryPtr->dependent_count;
                                }
                            ...
                            if ((scs_ptr->enable_altrefs == EB_TRUE &&
                                ...
                                // Start Filtering in ME processes
                                {
                                    ...
                                    for (seg_idx = 0; seg_idx < pcs_ptr->tf_segments_total_count; ++seg_idx) {
                                        ...
                                        out_results_ptr = (PictureDecisionResults*)outputResultsWrapperPtr->object_ptr;
                                        out_results_ptr->picture_control_set_wrapper_ptr = encode_context_ptr->pre_assignment_buffer[pictureIndex];
                        ...
                        for (pictureIndex = context_ptr->mini_gop_start_index[mini_gop_index]; pictureIndex <= context_ptr->mini_gop_end_index[mini_gop_index] + has_overlay; ++pictureIndex) {
                            ...
                            do {
                                ...
                                inputQueueIndex = (inputQueueIndex == PICTURE_DECISION_PA_REFERENCE_QUEUE_MAX_DEPTH - 1) ? 0 : inputQueueIndex + 1;
                            } while ((inputQueueIndex != encode_context_ptr->picture_decision_pa_reference_queue_tail_index) && (inputEntryPtr->picture_number != pcs_ptr->picture_number));
                            ...
                            if ((pcs_ptr->slice_type == P_SLICE) || (pcs_ptr->slice_type == B_SLICE)) {
                                ...
                                for (ref_pic_index = 0; ref_pic_index < pcs_ptr->ref_list0_count; ++ref_pic_index) {
                                    if (pcs_ptr->ref_list0_count) {
                                        ...
                                        pcs_ptr->ref_pa_pic_ptr_array[REF_LIST_0][ref_pic_index] = paReferenceEntryPtr->input_object_ptr;
                                ...
                                for (ref_pic_index = 0; ref_pic_index < pcs_ptr->ref_list1_count; ++ref_pic_index) {
                                    if (pcs_ptr->ref_list1_count) {
                                        ...
                                        ref_poc = POC_CIRCULAR_ADD(
                                            ...
                                            scs_ptr->bits_for_picture_order_count*/);
                                        // Set the Reference Object
                                        pcs_ptr->ref_pa_pic_ptr_array[REF_LIST_1][ref_pic_index] = paReferenceEntryPtr->input_object_ptr;
