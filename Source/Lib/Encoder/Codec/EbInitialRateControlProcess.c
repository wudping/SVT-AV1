    ...
    int32_t                        *yCurrentMv)
{
    ...
    const MeLcuResults *me_results = pcs_ptr->me_results[sb_index];
    ...
    PictureParentControlSet    *pcs_ptr)
{
    ...
    for (sb_count = 0; sb_count < pcs_ptr->sb_total_count; ++sb_count) {
        ...
        if (((sb_origin_x + BLOCK_SIZE_64) <= pcs_ptr->enhanced_picture_ptr->width) &&
            ...
            // Current MV
            GetMv(pcs_ptr, sb_count, &xCurrentMv, &yCurrentMv);
            ...
            if (sb_origin_x == 0) {
                ...
                GetMv(pcs_ptr, sb_count - 1, &xLeftMv, &yLeftMv);
            ...
            if (sb_origin_y == 0) {
                ...
                GetMv(pcs_ptr, sb_count - picture_width_in_sb, &xTopMv, &yTopMv);
            ...
            if ((sb_origin_x + (BLOCK_SIZE_64 << 1)) > pcs_ptr->enhanced_picture_ptr->width) {
                ...
                GetMv(pcs_ptr, sb_count + 1, &xRightMv, &yRightMv);
            ...
            if ((sb_origin_y + (BLOCK_SIZE_64 << 1)) > pcs_ptr->enhanced_picture_ptr->height) {
                ...
                GetMv(pcs_ptr, sb_count + picture_width_in_sb, &xBottomMv, &yBottomMv);
    ...
    PictureParentControlSet    *pcs_ptr)
{
    ...
    for (uint32_t listIndex = REF_LIST_0; listIndex <= numOfListToSearch; ++listIndex) {
        ...
        for (uint32_t ref_pic_index = 0; ref_pic_index < num_of_ref_pic_to_search;
             ++ref_pic_index)
        {
            pcs_ptr->is_global_motion[listIndex][ref_pic_index] = EB_FALSE;
            if (pcs_ptr->global_motion_estimation[listIndex][ref_pic_index].wmtype > TRANSLATION)
                pcs_ptr->is_global_motion[listIndex][ref_pic_index] = EB_TRUE;
        }
    }
#endif
    ...
    for (sb_count = 0; sb_count < pcs_ptr->sb_total_count; ++sb_count) {
        ...
        if (((sb_origin_x + BLOCK_SIZE_64) <= pcs_ptr->enhanced_picture_ptr->width) &&
            ...
            if ((EbBool)(CheckMvForPan(pcs_ptr->hierarchical_levels, pcs_ptr->temporal_layer_index, &xCurrentMv, &yCurrentMv, &xLeftMv, &yLeftMv) ||
                CheckMvForPan(pcs_ptr->hierarchical_levels, pcs_ptr->temporal_layer_index, &xCurrentMv, &yCurrentMv, &xTopMv, &yTopMv) ||
                CheckMvForPan(pcs_ptr->hierarchical_levels, pcs_ptr->temporal_layer_index, &xCurrentMv, &yCurrentMv, &xRightMv, &yRightMv) ||
                CheckMvForPan(pcs_ptr->hierarchical_levels, pcs_ptr->temporal_layer_index, &xCurrentMv, &yCurrentMv, &xBottomMv, &yBottomMv))) {
                totalPanLcus++;
            ...
            if ((EbBool)(CheckMvForTilt(pcs_ptr->hierarchical_levels, pcs_ptr->temporal_layer_index, &xCurrentMv, &yCurrentMv, &xLeftMv, &yLeftMv) ||
                CheckMvForTilt(pcs_ptr->hierarchical_levels, pcs_ptr->temporal_layer_index, &xCurrentMv, &yCurrentMv, &xTopMv, &yTopMv) ||
                CheckMvForTilt(pcs_ptr->hierarchical_levels, pcs_ptr->temporal_layer_index, &xCurrentMv, &yCurrentMv, &xRightMv, &yRightMv) ||
                CheckMvForTilt(pcs_ptr->hierarchical_levels, pcs_ptr->temporal_layer_index, &xCurrentMv, &yCurrentMv, &xBottomMv, &yBottomMv))) {
                totalTiltLcus++;
            ...
            if ((EbBool)(CheckMvForPanHighAmp(pcs_ptr->hierarchical_levels, pcs_ptr->temporal_layer_index, &xCurrentMv, &xLeftMv) ||
                CheckMvForPanHighAmp(pcs_ptr->hierarchical_levels, pcs_ptr->temporal_layer_index, &xCurrentMv, &xTopMv) ||
                CheckMvForPanHighAmp(pcs_ptr->hierarchical_levels, pcs_ptr->temporal_layer_index, &xCurrentMv, &xRightMv) ||
                CheckMvForPanHighAmp(pcs_ptr->hierarchical_levels, pcs_ptr->temporal_layer_index, &xCurrentMv, &xBottomMv))) {
                totalPanHighAmpLcus++;
            }

            if ((EbBool)(CheckMvForTiltHighAmp(pcs_ptr->hierarchical_levels, pcs_ptr->temporal_layer_index, &yCurrentMv, &yLeftMv) ||
                CheckMvForTiltHighAmp(pcs_ptr->hierarchical_levels, pcs_ptr->temporal_layer_index, &yCurrentMv, &yTopMv) ||
                CheckMvForTiltHighAmp(pcs_ptr->hierarchical_levels, pcs_ptr->temporal_layer_index, &yCurrentMv, &yRightMv) ||
                CheckMvForTiltHighAmp(pcs_ptr->hierarchical_levels, pcs_ptr->temporal_layer_index, &yCurrentMv, &yBottomMv))) {
    ...
    if ((totalCheckedLcus > 0) && ((totalPanLcus * 100 / totalCheckedLcus) > PAN_LCU_PERCENTAGE)) {
        ...
        if (totalPanLcus > 0) {
            pcs_ptr->panMvx = (int16_t)(xPanMvSum / totalPanLcus);
            pcs_ptr->panMvy = (int16_t)(yPanMvSum / totalPanLcus);
        }
    }
    ...
    if ((totalCheckedLcus > 0) && ((totalTiltLcus * 100 / totalCheckedLcus) > PAN_LCU_PERCENTAGE)) {
        ...
        if (totalTiltLcus > 0) {
            pcs_ptr->tiltMvx = (int16_t)(xTiltMvSum / totalTiltLcus);
            pcs_ptr->tiltMvy = (int16_t)(yTiltMvSum / totalTiltLcus);
        }
    }
#if GM_OPT && GLOBAL_WARPED_MOTION
    }
    ...
    PictureParentControlSet         *pcs_ptr)
{
    ...
    if (pcs_ptr->slice_type != I_SLICE) {
        numOfListToSearch = (pcs_ptr->slice_type == P_SLICE) ? REF_LIST_0 : REF_LIST_1;
        ...
        for (listIndex = REF_LIST_0; listIndex <= numOfListToSearch; ++listIndex) {
            ...
            for (ref_pic_index = 0; ref_pic_index < num_of_ref_pic_to_search; ++ref_pic_index) {
                if (pcs_ptr->ref_pa_pic_ptr_array[listIndex][ref_pic_index] != EB_NULL) {
                    eb_release_object(pcs_ptr->ref_pa_pic_ptr_array[listIndex][ref_pic_index]);
                }
            }
        }
    }
    ...
    PictureParentControlSet         *pcs_ptr)
{
    ...
    if (pcs_ptr->slice_type != I_SLICE && pcs_ptr->temporal_layer_index == 0)
        CheckForNonUniformMotionVectorField(pcs_ptr);
    return;
}
    ...
    for (sb_index = 0; sb_index < pcs_ptr->sb_total_count; sb_index++) {
        ...
        if (sb_params.potential_logo_sb &&sb_params.is_complete_sb) {
            ...
            if (pcs_ptr->temporal_layer_index > 0)
                GetMv(pcs_ptr, sb_index, &xCurrentMv, &yCurrentMv);

            EbBool lowMotion = pcs_ptr->temporal_layer_index == 0 ? EB_TRUE : (ABS(xCurrentMv) < 16) && (ABS(yCurrentMv) < 16) ? EB_TRUE : EB_FALSE;
            uint16_t *yVariancePtr = pcs_ptr->variance[sb_index];
    ...
    PictureParentControlSet   *pcs_ptr)
{
    ...
    uint32_t               lowSadTh = (scs_ptr->input_resolution < INPUT_SIZE_1080p_RANGE) ? 5 : 2;
    ...
    for (sb_index = 0; sb_index < pcs_ptr->sb_total_count; sb_index++) {
        ...
        if (sb_params.potential_logo_sb &&sb_params.is_complete_sb) {
            ...
            if (pcs_ptr->slice_type == B_SLICE)
                GetMeDist(pcs_ptr, sb_index, &meDist);
            lowSad = (pcs_ptr->slice_type != B_SLICE) ?
    ...
    uint32_t                       totalLcuCount)
{
    ...
    for (sb_index = 0; sb_index < totalLcuCount; sb_index++) {
        ...
        if (sb_params.potential_logo_sb &&sb_params.is_complete_sb && sb_stat_ptr->check1_for_logo_stationary_edge_over_time_flag && sb_stat_ptr->check2_for_logo_stationary_edge_over_time_flag) {
            uint32_t rasterScanCuIndex;
            uint32_t similar_edge_count_lcu = 0;
            // CU Loop
            for (rasterScanCuIndex = RASTER_SCAN_CU_INDEX_16x16_0; rasterScanCuIndex <= RASTER_SCAN_CU_INDEX_16x16_15; rasterScanCuIndex++)
                similar_edge_count_lcu += (sb_stat_ptr->cu_stat_array[rasterScanCuIndex].similar_edge_count > slideWindowTh) ? 1 : 0;
            sb_stat_ptr->stationary_edge_over_time_flag = (similar_edge_count_lcu >= 4) ? EB_TRUE : EB_FALSE;
        }
    }

    {
        ...
        uint32_t picture_width_in_sb = scs_ptr->picture_width_in_sb;
                ...
                if (sb_stat_ptr->stationary_edge_over_time_flag == 0 && sb_params.potential_logo_sb && (sb_stat_ptr->check2_for_logo_stationary_edge_over_time_flag || !sb_params.is_complete_sb)) {
                    ...
                    for (lcuVer = lcuVerS; lcuVer <= lcuVerE; lcuVer++) {
                        ...
                        for (lcuHor = lcuHorS; lcuHor <= lcuHorE; lcuHor++)
                            countOfNeighbors += (pcs_ptr->sb_stat_array[sb_index + lcuVerOffset + lcuHor].stationary_edge_over_time_flag == 1);
                    }
...

            {
                if (sb_stat_ptr->stationary_edge_over_time_flag == 0 && sb_params.potential_logo_sb && (sb_stat_ptr->check2_for_logo_stationary_edge_over_time_flag || !sb_params.is_complete_sb)) {
                    ...
                    for (lcuVer = lcuVerS; lcuVer <= lcuVerE; lcuVer++) {
                        lcuVerOffset = lcuVer * (int32_t)picture_width_in_sb;
                        for (lcuHor = lcuHorS; lcuHor <= lcuHorE; lcuHor++)
                            countOfNeighbors += (pcs_ptr->sb_stat_array[sb_index + lcuVerOffset + lcuHor].stationary_edge_over_time_flag == 2);
                    }
    ...
    PictureParentControlSet         *pcs_ptr)
{
    ...
    updateIsPanFramesToCheck = MIN(8, pcs_ptr->frames_in_sw);
    ...
    PictureParentControlSet         *pcs_ptr)
{
    ...
    updateNonMovingIndexArrayFramesToCheck = MIN(MIN(((pcs_ptr->pred_struct_ptr->pred_struct_period << 1) + 1), pcs_ptr->frames_in_sw), scs_ptr->static_config.look_ahead_distance);
    uint64_t me_dist = 0;
    uint8_t me_dist_pic_count = 0;
    // SB Loop
    for (lcuIdx = 0; lcuIdx < pcs_ptr->sb_total_count; ++lcuIdx) {
        uint16_t nonMovingIndexOverSlidingWindow = pcs_ptr->non_moving_index_array[lcuIdx];
        ...
        for (framesToCheckIndex = 0; framesToCheckIndex < updateNonMovingIndexArrayFramesToCheck - 1; framesToCheckIndex++) {
            ...
            if (temporaryPictureControlSetPtr->temporal_layer_index < MAX((int8_t)pcs_ptr->hierarchical_levels - 1, 2) ) {
                if (lcuIdx == 0)
                    me_dist_pic_count++;
            ...
            if (pcs_ptr->slice_type == I_SLICE && pcs_ptr->filtered_sse == 0 && lcuIdx == 0 && temporaryPictureControlSetPtr->temporal_layer_index == 0) {
                pcs_ptr->filtered_sse = temporaryPictureControlSetPtr->filtered_sse;
                pcs_ptr->filtered_sse_uv = temporaryPictureControlSetPtr->filtered_sse_uv;
            }
        ...
        pcs_ptr->non_moving_index_array[lcuIdx] = (uint8_t)(nonMovingIndexOverSlidingWindow / (framesToCheckIndex + 1));

        nonMovingIndexSum += pcs_ptr->non_moving_index_array[lcuIdx];
    }

    pcs_ptr->non_moving_index_average = (uint16_t)nonMovingIndexSum / pcs_ptr->sb_total_count;
    me_dist_pic_count = MAX(me_dist_pic_count, 1);
    pcs_ptr->qp_scaling_average_complexity = (uint16_t)((uint64_t)me_dist / pcs_ptr->sb_total_count / 256 / me_dist_pic_count);
    return;
}
    ...
    PictureParentControlSet         *pcs_ptr)
{
    ...
    // SB Loop
    for (lcuIdx = 0; lcuIdx < pcs_ptr->sb_total_count; ++lcuIdx)
        pcs_ptr->non_moving_index_array[lcuIdx] = INVALID_ZZ_COST;
    return;
}
    ...
    PictureParentControlSet         *pcs_ptr)
{
    ...
    // Determine number of frames to check N
    NoFramesToCheck = MIN(MIN(((pcs_ptr->pred_struct_ptr->pred_struct_period << 1) + 1), pcs_ptr->frames_in_sw), scs_ptr->static_config.look_ahead_distance);
    ...
    MotionEstimationResults         *in_results_ptr)
{
    ...
    queueEntryIndex = (int32_t)(pcs_ptr->picture_number - encode_context_ptr->initial_rate_control_reorder_queue[encode_context_ptr->initial_rate_control_reorder_queue_head_index]->picture_number);
    ...
    queueEntryPtr->parent_pcs_wrapper_ptr = in_results_ptr->picture_control_set_wrapper_ptr;
    queueEntryPtr->picture_number = pcs_ptr->picture_number;
    ...
    PictureParentControlSet         *pcs_ptr)
{
    ...
    histogramQueueEntryIndex = (int32_t)(pcs_ptr->picture_number - encode_context_ptr->hl_rate_control_historgram_queue[encode_context_ptr->hl_rate_control_historgram_queue_head_index]->picture_number);
    ...
    //histogramQueueEntryPtr->parent_pcs_wrapper_ptr  = in_results_ptr->picture_control_set_wrapper_ptr;
    histogramQueueEntryPtr->picture_number = pcs_ptr->picture_number;
    histogramQueueEntryPtr->end_of_sequence_flag = pcs_ptr->end_of_sequence_flag;
    histogramQueueEntryPtr->slice_type = pcs_ptr->slice_type;
    histogramQueueEntryPtr->temporal_layer_index = pcs_ptr->temporal_layer_index;
    histogramQueueEntryPtr->full_sb_count = pcs_ptr->full_sb_count;
    ...
    //printf("Test1 POC: %d\t POC: %d\t LifeCount: %d\n", histogramQueueEntryPtr->picture_number, pcs_ptr->picture_number,  histogramQueueEntryPtr->life_count);
    ...
    uint32_t                           frames_in_sw)
{
    ...
    if (scs_ptr->static_config.rate_control_mode == 2)
        histogramQueueEntryPtr->life_count += (int16_t)(scs_ptr->static_config.intra_period_length + 1) - 3; // FramelevelRC does not decrease the life count for first picture in each temporal layer
    else
        histogramQueueEntryPtr->life_count += pcs_ptr->historgram_life_count;
...
void* initial_rate_control_kernel(void *input_ptr)
{
    ...
    for (;;) {
        ...
        in_results_ptr = (MotionEstimationResults*)inputResultsWrapperPtr->object_ptr;
        ...
        if (SEGMENT_COMPLETION_MASK_TEST(pcs_ptr->me_segments_completion_mask, pcs_ptr->me_segments_total_count)) {
            ...
            while (moveSlideWondowFlag) {
                ...
                while (moveSlideWondowFlag && !end_of_sequence_flag &&
                    queueEntryIndexTemp <= encode_context_ptr->initial_rate_control_reorder_queue_head_index + scs_ptr->static_config.look_ahead_distance) {
                ...
                if (moveSlideWondowFlag) {
                    ...
                    pcs_ptr = ((PictureParentControlSet*)(queueEntryPtr->parent_pcs_wrapper_ptr)->object_ptr);
                    scs_ptr = (SequenceControlSet*)pcs_ptr->sequence_control_set_wrapper_ptr->object_ptr;
                    ...
                    for (uint8_t loop_index = 0; loop_index <= has_overlay; loop_index++) {
                        ...
                        while (!end_of_sequence_flag &&
                            ...
                            if (scs_ptr->intra_period_length != -1) {
                                if (pcs_ptr->picture_number % ((scs_ptr->intra_period_length + 1)) == 0) {
                                    pcs_ptr->frames_in_interval[pictureControlSetPtrTemp->temporal_layer_index] ++;
                        ...
                        if ((scs_ptr->static_config.look_ahead_distance != 0) && (frames_in_sw < (scs_ptr->static_config.look_ahead_distance + 1)))
                        ...
                        if (!pcs_ptr->end_of_sequence_flag && scs_ptr->static_config.look_ahead_distance != 0) {
...
#if TWO_PASS
                        pcs_ptr->stat_struct_first_pass_ptr = pcs_ptr->is_used_as_reference_flag ? &((EbReferenceObject*)pcs_ptr->reference_picture_wrapper_ptr->object_ptr)->stat_struct : &pcs_ptr->stat_struct;
                        ...
                        out_results_ptr = (InitialRateControlResults*)outputResultsWrapperPtr->object_ptr;

                        if (loop_index)
                            out_results_ptr->picture_control_set_wrapper_ptr = pcs_ptr->p_pcs_wrapper_ptr;
                        else
                        out_results_ptr->picture_control_set_wrapper_ptr = queueEntryPtr->parent_pcs_wrapper_ptr;
