...
EbErrorType signal_derivation_pre_analysis_oq(
    ...
    uint8_t  hme_me_level = scs_ptr->use_output_stat_file ? pcs_ptr->snd_pass_enc_mode : pcs_ptr->enc_mode;
    // Derive HME Flag
    if (scs_ptr->static_config.use_default_me_hme) {
        pcs_ptr->enable_hme_flag = enable_hme_flag[0][input_resolution][hme_me_level] || enable_hme_flag[1][input_resolution][hme_me_level];
        pcs_ptr->enable_hme_level0_flag = enable_hme_level0_flag[0][input_resolution][hme_me_level] || enable_hme_level0_flag[1][input_resolution][hme_me_level];
        pcs_ptr->enable_hme_level1_flag = enable_hme_level1_flag[0][input_resolution][hme_me_level] || enable_hme_level1_flag[1][input_resolution][hme_me_level];
        pcs_ptr->enable_hme_level2_flag = enable_hme_level2_flag[0][input_resolution][hme_me_level] || enable_hme_level2_flag[1][input_resolution][hme_me_level];
    }
    else {
    ...
    pcs_ptr->tf_enable_hme_flag = tf_enable_hme_flag[0][input_resolution][hme_me_level] || tf_enable_hme_flag[1][input_resolution][hme_me_level];
    pcs_ptr->tf_enable_hme_level0_flag = tf_enable_hme_level0_flag[0][input_resolution][hme_me_level] || tf_enable_hme_level0_flag[1][input_resolution][hme_me_level];
    pcs_ptr->tf_enable_hme_level1_flag = tf_enable_hme_level1_flag[0][input_resolution][hme_me_level] || tf_enable_hme_level1_flag[1][input_resolution][hme_me_level];
    pcs_ptr->tf_enable_hme_level2_flag = tf_enable_hme_level2_flag[0][input_resolution][hme_me_level] || tf_enable_hme_level2_flag[1][input_resolution][hme_me_level];
    ...
    if (scs_ptr->static_config.enable_restoration_filtering == DEFAULT) {
        ...
        scs_ptr->seq_header.enable_restoration = (uint8_t)scs_ptr->static_config.enable_restoration_filtering;
    ...
    SequenceControlSet            *scs_ptr)
{
    ...
    int8_t changeCond = 0;
    int64_t targetFps = (scs_ptr->static_config.injector_frame_rate >> 16);
    ...
    inputFramesCount = (int64_t)overallDuration *(scs_ptr->static_config.injector_frame_rate >> 16) / 1000;
    scs_ptr->encode_context_ptr->sc_buffer = inputFramesCount - scs_ptr->encode_context_ptr->sc_frame_in;

    encoderModeDelta = 0;

    // Check every bufferTsshold1 for the changes (previous_frame_in_check1 variable)
    if ((scs_ptr->encode_context_ptr->sc_frame_in > context_ptr->previous_frame_in_check1 + bufferTrshold1 && scs_ptr->encode_context_ptr->sc_frame_in >= SC_FRAMES_TO_IGNORE)) {
        // Go to a slower mode based on the fullness and changes of the buffer
        if (scs_ptr->encode_context_ptr->sc_buffer < targetFps && (context_ptr->prev_enc_mode_delta > -1 || (context_ptr->prev_enc_mode_delta < 0 && scs_ptr->encode_context_ptr->sc_frame_in > context_ptr->previous_mode_change_frame_in + targetFps * 2))) {
            if (context_ptr->previous_buffer_check1 > scs_ptr->encode_context_ptr->sc_buffer + bufferTrshold1) {
                encoderModeDelta += -1;
                changeCond = 2;
            }
            else if (context_ptr->previous_mode_change_buffer > bufferTrshold1 + scs_ptr->encode_context_ptr->sc_buffer && scs_ptr->encode_context_ptr->sc_buffer < bufferTrshold1) {
                encoderModeDelta += -1;
                changeCond = 4;
            }
        }
        ...
        if (scs_ptr->encode_context_ptr->sc_buffer > bufferTrshold1 + context_ptr->previous_buffer_check1) {
            encoderModeDelta += +1;
            changeCond = 1;
        }
        else if (scs_ptr->encode_context_ptr->sc_buffer > bufferTrshold1 + context_ptr->previous_mode_change_buffer) {
            encoderModeDelta += +1;
            changeCond = 3;
        }
        ...
        if (scs_ptr->encode_context_ptr->sc_buffer > bufferTrshold3 &&
            (context_ptr->prev_change_cond != 7 || scs_ptr->encode_context_ptr->sc_frame_in > context_ptr->previous_mode_change_frame_in + bufferTrshold2 * 2) &&
            scs_ptr->encode_context_ptr->sc_buffer > context_ptr->previous_mode_change_buffer) {
            encoderModeDelta += 1;
            changeCond = 7;
        }
        encoderModeDelta = CLIP3(-1, 1, encoderModeDelta);
        scs_ptr->encode_context_ptr->enc_mode = (EbEncMode)CLIP3(1, MAX_ENC_PRESET, (int8_t)scs_ptr->encode_context_ptr->enc_mode + encoderModeDelta);
    ...
    if ((scs_ptr->encode_context_ptr->sc_frame_in > context_ptr->previous_frame_in_check2 + bufferTrshold2 && scs_ptr->encode_context_ptr->sc_frame_in >= SC_FRAMES_TO_IGNORE)) {
        encoderModeDelta = 0;

        // if no change in the encoder mode and buffer is low enough and level is not increasing, switch to a slower encoder mode
        // If previous ChangeCond was the same, double the threshold2
        if (encoderModeDelta == 0 && scs_ptr->encode_context_ptr->sc_frame_in > context_ptr->previous_mode_change_frame_in + bufferTrshold2 &&
            (context_ptr->prev_change_cond != 8 || scs_ptr->encode_context_ptr->sc_frame_in > context_ptr->previous_mode_change_frame_in + bufferTrshold2 * 2) &&
            ((scs_ptr->encode_context_ptr->sc_buffer - context_ptr->previous_mode_change_buffer < (targetFps / 3)) || context_ptr->previous_mode_change_buffer == 0) &&
            scs_ptr->encode_context_ptr->sc_buffer < bufferTrshold3) {
            encoderModeDelta = -1;
            changeCond = 8;
        }
    ...
    if (context_ptr->start_flag || (scs_ptr->encode_context_ptr->sc_frame_in > context_ptr->previous_frame_in_check3 + SC_FRAMES_INTERVAL_SPEED && scs_ptr->encode_context_ptr->sc_frame_in >= SC_FRAMES_TO_IGNORE)) {
        if (context_ptr->start_flag)
            context_ptr->cur_speed = (uint64_t)(scs_ptr->encode_context_ptr->sc_frame_out - 0) * 1000 / (uint64_t)(overallDuration);
        else {
            if (instDuration != 0)
                context_ptr->cur_speed = (uint64_t)(scs_ptr->encode_context_ptr->sc_frame_out - context_ptr->prev_frame_out) * 1000 / (uint64_t)(instDuration);
        }
        context_ptr->start_flag = EB_FALSE;
    ...
    else if (scs_ptr->encode_context_ptr->sc_frame_in < SC_FRAMES_TO_IGNORE && (overallDuration != 0))
        context_ptr->cur_speed = (uint64_t)(scs_ptr->encode_context_ptr->sc_frame_out - 0) * 1000 / (uint64_t)(overallDuration);
    if (changeCond)
...
void ResetPcsAv1(
    ...
    for (int32_t i = 0; i < REF_FRAMES; i++)
        pcs_ptr->fb_of_context_type[i] = 0;
    ...
    uint8_t                          *src)
{
    ...
    if (!is16_bit_input) {
        uint32_t     lumaBufferOffset = (dst_picture_ptr->stride_y*scs_ptr->top_padding + scs_ptr->left_padding) << is16_bit_input;
        uint32_t     chromaBufferOffset = (dst_picture_ptr->stride_cr*(scs_ptr->top_padding >> 1) + (scs_ptr->left_padding >> 1)) << is16_bit_input;
        uint16_t     lumaStride = dst_picture_ptr->stride_y << is16_bit_input;
        uint16_t     chromaStride = dst_picture_ptr->stride_cb << is16_bit_input;
        uint16_t     lumaWidth = (uint16_t)(dst_picture_ptr->width - scs_ptr->max_input_pad_right) << is16_bit_input;
        uint16_t     chromaWidth = (lumaWidth >> 1) << is16_bit_input;
        uint16_t     lumaHeight = (uint16_t)(dst_picture_ptr->height - scs_ptr->max_input_pad_bottom);
    ...
    {
        {
            uint32_t  lumaBufferOffset = (dst_picture_ptr->stride_y*scs_ptr->top_padding + scs_ptr->left_padding);
            uint32_t  chromaBufferOffset = (dst_picture_ptr->stride_cr*(scs_ptr->top_padding >> 1) + (scs_ptr->left_padding >> 1));
            uint16_t  lumaStride = dst_picture_ptr->stride_y;
            uint16_t  chromaStride = dst_picture_ptr->stride_cb;
            uint16_t  lumaWidth = (uint16_t)(dst_picture_ptr->width - scs_ptr->max_input_pad_right);
    ...
    SequenceControlSet       *scs_ptr)
{
    ...
    int32_t fseek_return_value = fseek(scs_ptr->static_config.input_stat_file, (long)pcs_ptr->picture_number * sizeof(StatStruct), SEEK_SET);
    ...
    for (int sb_addr = 0; sb_addr < scs_ptr->sb_total_count; ++sb_addr) {
        referenced_area_avg += (pcs_ptr->stat_struct.referenced_area[sb_addr] / scs_ptr->sb_params_array[sb_addr].width / scs_ptr->sb_params_array[sb_addr].height);
    ...
    if (scs_ptr->intra_period_length && scs_ptr->intra_period_length < TWO_PASS_IR_THRSHLD)
        referenced_area_avg = referenced_area_avg * (scs_ptr->intra_period_length + 1) / TWO_PASS_IR_THRSHLD;
...
void* resource_coordination_kernel(void *input_ptr)
{
    ...
    for (;;) {
        ...
        scs_ptr = context_ptr->sequence_control_set_instance_array[instance_index]->scs_ptr;
        ...
        if (context_ptr->sequence_control_set_instance_array[instance_index]->encode_context_ptr->initial_picture) {
            ...
            {
                context_ptr->sequence_control_set_instance_array[instance_index]->scs_ptr->seq_header.max_frame_width = context_ptr->sequence_control_set_instance_array[instance_index]->scs_ptr->max_input_luma_width;
                context_ptr->sequence_control_set_instance_array[instance_index]->scs_ptr->seq_header.max_frame_height = context_ptr->sequence_control_set_instance_array[instance_index]->scs_ptr->max_input_luma_height;
                context_ptr->sequence_control_set_instance_array[instance_index]->scs_ptr->chroma_width = (context_ptr->sequence_control_set_instance_array[instance_index]->scs_ptr->max_input_luma_width >> 1);
                context_ptr->sequence_control_set_instance_array[instance_index]->scs_ptr->chroma_height = (context_ptr->sequence_control_set_instance_array[instance_index]->scs_ptr->max_input_luma_height >> 1);

                context_ptr->sequence_control_set_instance_array[instance_index]->scs_ptr->pad_right = context_ptr->sequence_control_set_instance_array[instance_index]->scs_ptr->max_input_pad_right;
                context_ptr->sequence_control_set_instance_array[instance_index]->scs_ptr->cropping_right_offset = context_ptr->sequence_control_set_instance_array[instance_index]->scs_ptr->pad_right;
                context_ptr->sequence_control_set_instance_array[instance_index]->scs_ptr->pad_bottom = context_ptr->sequence_control_set_instance_array[instance_index]->scs_ptr->max_input_pad_bottom;
                context_ptr->sequence_control_set_instance_array[instance_index]->scs_ptr->cropping_bottom_offset = context_ptr->sequence_control_set_instance_array[instance_index]->scs_ptr->pad_bottom;

                if (context_ptr->sequence_control_set_instance_array[instance_index]->scs_ptr->pad_right != 0 || context_ptr->sequence_control_set_instance_array[instance_index]->scs_ptr->pad_bottom != 0)
                    context_ptr->sequence_control_set_instance_array[instance_index]->scs_ptr->conformance_window_flag = 1;
                else
                    context_ptr->sequence_control_set_instance_array[instance_index]->scs_ptr->conformance_window_flag = 0;
                input_size = context_ptr->sequence_control_set_instance_array[instance_index]->scs_ptr->seq_header.max_frame_width * context_ptr->sequence_control_set_instance_array[instance_index]->scs_ptr->seq_header.max_frame_height;
            }
            ...
            copy_sequence_control_set(
                ...
                context_ptr->sequence_control_set_instance_array[instance_index]->scs_ptr);
        ...
        scs_ptr = (SequenceControlSet*)context_ptr->sequenceControlSetActiveArray[instance_index]->object_ptr;
        ...
        if (context_ptr->sequence_control_set_instance_array[instance_index]->encode_context_ptr->initial_picture) {
...
#else
            scs_ptr->enable_altrefs = scs_ptr->static_config.enable_altrefs &&
                scs_ptr->static_config.altref_nframes > 1 &&
                ((scs_ptr->static_config.encoder_bit_depth >= 8 && scs_ptr->static_config.enc_mode == ENC_M0) ||
                    scs_ptr->static_config.encoder_bit_depth == 8) ? EB_TRUE : EB_FALSE;
#endif
...
#if M0_OPT
#if PRESETS_TUNE
                scs_ptr->seq_header.enable_interintra_compound = MR_MODE || (scs_ptr->static_config.enc_mode <= ENC_M1 && scs_ptr->static_config.screen_content_mode != 1) ? 1 : 0;
#else
                scs_ptr->seq_header.enable_interintra_compound = MR_MODE || (scs_ptr->static_config.enc_mode == ENC_M0 && scs_ptr->static_config.screen_content_mode != 1) ? 1 : 0;
#endif
#else
            scs_ptr->seq_header.enable_interintra_compound = (scs_ptr->static_config.enc_mode == ENC_M0) ? 1 : 0;
#endif
            ...
            if (scs_ptr->static_config.enable_filter_intra)
                scs_ptr->seq_header.enable_filter_intra = (scs_ptr->static_config.enc_mode <= ENC_M4) ? 1 : 0;
            else
        ...
        uint8_t has_overlay = (scs_ptr->static_config.enable_overlays == EB_FALSE ||
        ...
        for (uint8_t loop_index = 0; loop_index <= has_overlay && !end_of_sequence_flag; loop_index++) {
            ...
            pcs_ptr = (PictureParentControlSet*)picture_control_set_wrapper_ptr->object_ptr;

            pcs_ptr->p_pcs_wrapper_ptr = picture_control_set_wrapper_ptr;
            ...
            if (loop_index) {
                ...
                pcs_ptr->alt_ref_ppcs_ptr = ((PictureParentControlSet*)alt_ref_picture_control_set_wrapper_ptr->object_ptr);
            ...
            pcs_ptr->previous_picture_control_set_wrapper_ptr = (context_ptr->sequence_control_set_instance_array[instance_index]->encode_context_ptr->initial_picture) ?
                picture_control_set_wrapper_ptr :
                scs_ptr->encode_context_ptr->previous_picture_control_set_wrapper_ptr;
            if (loop_index == 0)
                scs_ptr->encode_context_ptr->previous_picture_control_set_wrapper_ptr = picture_control_set_wrapper_ptr;
            // Copy data from the svt buffer to the input frame
            // *Note - Assumes 4:2:0 planar
            input_picture_wrapper_ptr = ebInputWrapperPtr;
            pcs_ptr->enhanced_picture_ptr = (EbPictureBufferDesc*)ebInputPtr->p_buffer;
            pcs_ptr->input_ptr = ebInputPtr;
            end_of_sequence_flag = (pcs_ptr->input_ptr->flags & EB_BUFFERFLAG_EOS) ? EB_TRUE : EB_FALSE;
            EbStartTime(&pcs_ptr->start_time_seconds, &pcs_ptr->start_time_u_seconds);

            pcs_ptr->sequence_control_set_wrapper_ptr = context_ptr->sequenceControlSetActiveArray[instance_index];
            ...
            if (loop_index == 1) {
                ...
                if (pcs_ptr->alt_ref_ppcs_ptr->input_picture_wrapper_ptr->object_ptr != NULL) {
                    CopyInputBuffer(
                        ...
                        (EbBufferHeaderType*)pcs_ptr->alt_ref_ppcs_ptr->input_picture_wrapper_ptr->object_ptr);
                }
            ...
            pcs_ptr->idr_flag = scs_ptr->encode_context_ptr->initial_picture || (pcs_ptr->input_ptr->pic_type == EB_AV1_KEY_PICTURE);
            pcs_ptr->cra_flag = (pcs_ptr->input_ptr->pic_type == EB_AV1_INTRA_ONLY_PICTURE) ? EB_TRUE : EB_FALSE;
            pcs_ptr->scene_change_flag = EB_FALSE;
            pcs_ptr->qp_on_the_fly = EB_FALSE;
            pcs_ptr->sb_total_count = scs_ptr->sb_total_count;
            pcs_ptr->eos_coming = (ebInputPtr->flags & (EB_BUFFERFLAG_EOS << 1)) ? EB_TRUE : EB_FALSE;
            ...
            pcs_ptr->snd_pass_enc_mode =
                ( scs_ptr->use_output_stat_file && scs_ptr->static_config.snd_pass_enc_mode != MAX_ENC_PRESET + 1)?
                (EbEncMode)scs_ptr->static_config.snd_pass_enc_mode : pcs_ptr->enc_mode;
            aspectRatio = (scs_ptr->seq_header.max_frame_width * 10) / scs_ptr->seq_header.max_frame_height;
            ...
            // Set the SCD Mode
            scs_ptr->scd_mode = scs_ptr->static_config.scene_change_detection == 0 ?
                SCD_MODE_0 :
                SCD_MODE_1;
            ...
            if (scs_ptr->static_config.rate_control_mode) {
                EB_MEMSET(pcs_ptr->me_distortion_histogram, 0, NUMBER_OF_SAD_INTERVALS * sizeof(uint16_t));
                EB_MEMSET(pcs_ptr->ois_distortion_histogram, 0, NUMBER_OF_INTRA_SAD_INTERVALS * sizeof(uint16_t));
            }
            ...
            if (pcs_ptr->is_overlay)
                ...
                eb_object_inc_live_count(
                    pcs_ptr->pa_reference_picture_wrapper_ptr,
                    1);
            else
                eb_object_inc_live_count(
                    pcs_ptr->pa_reference_picture_wrapper_ptr,
            ...
            if(scs_ptr->static_config.unrestricted_motion_vector == 0)
            {
                ...
                uint8_t picture_width_in_sb = (uint8_t)((scs_ptr->seq_header.max_frame_width + scs_ptr->sb_size_pix - 1) / scs_ptr->sb_size_pix);
                    ...
                    for (tile_col = 0; tile_col < tile_cols; tile_col++)
                    {
                             ...
                             y_lcu_index++)
                        {
                                 ...
                                 x_lcu_index++)
                            {
                                ...
                                scs_ptr->sb_params_array[sb_index].tile_start_x = 4 * tile_info.mi_col_start;
                                scs_ptr->sb_params_array[sb_index].tile_end_x   = 4 * tile_info.mi_col_end;
                                scs_ptr->sb_params_array[sb_index].tile_start_y = 4 * tile_info.mi_row_start;
                                scs_ptr->sb_params_array[sb_index].tile_end_y   = 4 * tile_info.mi_row_end;
                            }
                        }
                    }
                }
            ...
            if (pcs_ptr->picture_number > 0 && (prevPictureControlSetWrapperPtr != NULL))
            {
                ...
                eb_get_empty_object(
                    ...
                    &outputWrapperPtr);
                out_results_ptr = (ResourceCoordinationResults*)outputWrapperPtr->object_ptr;
                out_results_ptr->picture_control_set_wrapper_ptr = prevPictureControlSetWrapperPtr;
